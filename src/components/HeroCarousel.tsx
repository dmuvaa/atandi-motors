    ...
    <section className="overflow-hidden border-b border-[#e1eee2] bg-white">
      <div className="mx-auto max-w-7xl px-4 py-7 sm:px-6 lg:px-8 lg:py-10">
        <div className="relative aspect-[16/8] min-h-[330px] overflow-hidden rounded-[1.5rem] bg-[#f1f8ed] sm:min-h-[480px]">
          {slides.map((item, index) => <Image key={item.title} src={item.image} alt={item.alt} fill priority={index === 0} sizes="(max-width: 1280px) 100vw, 1280px" className={`object-cover transition-all duration-700 ${index === active ? "scale-100 opacity-100" : "scale-105 opacity-0"}`} />)}
        </div>
